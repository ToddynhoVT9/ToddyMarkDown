 ...
 */
export async function processAssets(assetPaths, // DocumentNode.assets
tmdFilePath, // caminho absoluto do .tmd
outputDir, // ex: /projeto/dist/meu-artigo/
filePath) {
 ...
 */
export function rewriteAssetPaths(html, assetResults) {
    let result = html;
    for (const asset of assetResults) {
        if (asset.found) {
            ...
            const imgRegex = new RegExp(`<img\\s+[^>]*src="${escapeRegex(asset.originalSrc)}"[^>]*/?>`, 'g');
