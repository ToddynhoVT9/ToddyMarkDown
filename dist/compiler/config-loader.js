...
const BASE_THEME_NAMES = new Set(VALID_THEMES);
/**
...
export function loadConfig(configPath) {
    ...
    if (config.themes && typeof config.themes === 'object') {
        for (const [name, themeDef] of Object.entries(config.themes)) {
            ...
            validateTheme(name, themeDef, config.themes);
            result.themes[name] = themeDef;
        }
...
function validateTheme(name, theme, allThemes) {
    ...
    const extendsTarget = theme.extends;
    ...
    if (theme.blocks) {
        for (const [blockType, blockProps] of Object.entries(theme.blocks)) {
...
function checkCircularExtends(themes) {
