...
const BASE_THEMES = ['essay', 'ink', 'modern', 'amber'];
/**
...
// ==================== Standalone ====================
function renderStandalone(doc, options) {
...
// ==================== Fragment ====================
function renderFragment(doc, options) {
...
// ==================== Switcher ====================
function renderSwitcher(customThemeNames) {
...
function renderHeader(fm) {
    const parts = [];
    if (fm.kicker)
        parts.push(`<p class="tmd-kicker">${escapeHTML(fm.kicker)}</p>`);
    if (fm.title)
        parts.push(`<h1 class="tmd-title">${escapeHTML(fm.title)}</h1>`);
    if (fm.subtitle)
        parts.push(`<p class="tmd-subtitle">${escapeHTML(fm.subtitle)}</p>`);
    if (fm.author)
        parts.push(`<p class="tmd-author">${escapeHTML(fm.author)}</p>`);
    if (parts.length === 0)
        return '';
...
function renderNode(node) {
    switch (node.type) {
        ...
        default: return renderContentBlock(node);
    }
}
// ==================== MarkdownBlock ====================
function renderMarkdownBlock(node) {
    return `<section class="tmd-markdown">${md.render(node.raw)}</section>`;
}
// ==================== LiteralBlock ====================
function renderLiteralBlock(node) {
...
// ==================== ErrorBlock ====================
function renderErrorBlock(node) {
