...
export function isContentBlockStart(line) {
    return CONTENT_BLOCK_OPEN_RE.test(line) || CONTENT_BLOCK_UNCLOSED_BRACKET_RE.test(line);
}
export function parseContentBlock(lines, startIndex, lineOffsets, filePath) {
    const diagnostics = [];
    ...
    if (CONTENT_BLOCK_UNCLOSED_BRACKET_RE.test(line)) {
        return makeErrorResult(lines, startIndex, startIndex, lineOffsets, filePath, diagnostics, DiagnosticCode.TITLE_BRACKET_NOT_CLOSED, 'Title bracket was not closed');
    }
    const match = CONTENT_BLOCK_OPEN_RE.exec(line);
    if (!match) {
        // Should not happen if isContentBlockStart was checked, but handle gracefully
        return makeErrorResult(lines, startIndex, startIndex, lineOffsets, filePath, diagnostics, DiagnosticCode.UNKNOWN_TOKEN, 'Failed to parse content block opening');
    }
    const token = match[1];
    let title = match[2] !== undefined ? match[2] : null;
    ...
    if (closingLine === -1) {
        ...
        return makeErrorResult(lines, startIndex, lastLine, lineOffsets, filePath, diagnostics, DiagnosticCode.BLOCK_NOT_CLOSED, 'Content block was not closed with <|');
    }
    const position = {
        start: positionAt(startIndex, 0, lineOffsets),
    ...
    // Dispatch by token
    if (token === '@') {
        const { node: pqNode, diagnostics: pqDiags } = parsePullQuote(bodyLines, title, lineOffsets, filePath, startIndex, bodyStartIndex, position);
        diagnostics.push(...pqDiags);
        return { node: pqNode, consumedLines: closingLine - startIndex + 1, diagnostics };
    }
    if (token === '~~') {
        const { node: tlNode, diagnostics: tlDiags } = parseTimeline(bodyLines, title, lineOffsets, filePath, bodyStartIndex, position);
    ...
    if (!blockType) {
        return makeErrorResult(lines, startIndex, closingLine, lineOffsets, filePath, diagnostics, DiagnosticCode.UNKNOWN_TOKEN, `Unknown content block token: "${token}"`);
    }
    ...
    if (bodyLines.length > 0) {
        ...
        content.push({ type: 'MarkdownBlock', raw: rawContent, position: mdPosition });
    }
    const node = {
        type: blockType,
        title,
...
// --- Pullquote ---
function parsePullQuote(bodyLines, title, lineOffsets, filePath, startLine, bodyStartIndex, position) {
    const diagnostics = [];
    // Empty pullquote
...
// --- Timeline ---
function parseTimeline(bodyLines, title, lineOffsets, filePath, bodyStartIndex, position) {
    const diagnostics = [];
    const items = [];
    let mdAccum = [];
    let mdStartLine = -1;
    function flushMarkdown(beforeLine) {
        if (mdAccum.length > 0) {
            const mdNode = {
...
// --- Error helper ---
function makeErrorResult(lines, startIndex, endIndex, lineOffsets, filePath, diagnostics, code, message) {
