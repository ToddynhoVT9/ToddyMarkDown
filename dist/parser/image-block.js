...
export const IMAGE_MODES = ['*>wrap', '*>', '*<wrap', '*<'];
// Atenção crítica: *>wrap e *<wrap DEVEM ser testados antes de *> e *<
// A ordem deste array é a ordem de tentativa no parser
export const IMAGE_BLOCK_OPEN_RE = /^\|>(\*>wrap|\*>|\*<wrap|\*<)(?:\s+\[(.*?)\])?\s+!\[(.*?)\]\((.*?)\)$/;
export function isImageBlockStart(line) {
 ...
 */
function isImageBlockWithErrors(line) {
    if (!line.startsWith('|>*'))
        return false;
...
export function parseImageBlock(lines, startIndex, lineOffsets, filePath) {
    const diagnostics = [];
    ...
    if (!mode) {
        ...
        return makeImageError(lines, startIndex, startIndex, lineOffsets, filePath, diagnostics, DiagnosticCode.IMAGE_INVALID_MODE, 'Invalid image mode');
    }
    ...
    if (!imageMatch) {
        return makeImageError(lines, startIndex, startIndex, lineOffsets, filePath, diagnostics, DiagnosticCode.IMAGE_MISSING, 'Image block has no image reference ![caption](path)');
    }
    ...
    // Validate caption
    if (caption === '') {
        return makeImageError(lines, startIndex, startIndex, lineOffsets, filePath, diagnostics, DiagnosticCode.IMAGE_NO_CAPTION, 'Image block has empty caption');
    }
    // Validate src
    if (src === '') {
        return makeImageError(lines, startIndex, startIndex, lineOffsets, filePath, diagnostics, DiagnosticCode.IMAGE_NO_SRC, 'Image block has empty src path');
    }
    // Step 5: check no second image on the line
    if (/!\[.*?\]\(.*?\)/.test(trailing)) {
        return makeImageError(lines, startIndex, startIndex, lineOffsets, filePath, diagnostics, DiagnosticCode.IMAGE_MULTIPLE, 'Image block has multiple images on the same line');
    }
    ...
    if (closingLine === -1) {
        ...
        return makeImageError(lines, startIndex, lastLine, lineOffsets, filePath, diagnostics, DiagnosticCode.BLOCK_NOT_CLOSED, 'Image block was not closed with <|');
    }
    const position = {
        start: positionAt(startIndex, 0, lineOffsets),
...
function makeImageError(lines, startIndex, endIndex, lineOffsets, filePath, diagnostics, code, message) {
