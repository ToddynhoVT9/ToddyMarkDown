import type { ResolvedConfig, TMDConfigTheme } from '../types/index.js';
...
function generateFontImports(config: ResolvedConfig): string {
  ...
  for (const name of Object.keys(BASE_THEMES)) {
    if (THEME_FONT_IMPORTS[name]) fontUrls.add(THEME_FONT_IMPORTS[name]);
  }
...
function generateAllThemeCSS(config: ResolvedConfig, mode: 'standalone' | 'fragment'): string {
  const parts: string[] = [];
...
function generateThemeVarsBlock(name: string, vars: Record<string, string>, mode: 'standalone' | 'fragment'): string {
...
const BLOCK_CSS_CLASS: Record<string, string> = {
...
function generateBlockOverrides(name: string, theme: TMDConfigTheme, mode: 'standalone' | 'fragment'): string {
...
function generateStructuralCSS(mode: 'standalone' | 'fragment'): string {
  const parts: string[] = [];
